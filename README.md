# 🤖 AI Jarvis - 智能每日任务助手

AI Jarvis 是一个基于 Claude AI 和 GitHub Actions 的智能个人助手系统，每天自动为你生成个性化的任务和提醒，通过邮件推送，让你的工作和生活更加高效有序。

## ✨ 主要功能

### 🔥 四大智能模块

1. **💼 工作提醒** - 基于你的 GitHub 活动生成工作任务
   - 代码审查提醒
   - 项目进度跟踪
   - 技术债务清理
   - 文档更新建议

2. **📚 学习计划** - 个性化技术学习建议
   - 技术栈深度学习
   - 新技术趋势探索
   - 实践项目推荐
   - 学习资源推荐

3. **💪 健康建议** - 程序员专项健康管理
   - 眼部健康保护
   - 颈椎脊椎护理
   - 运动健身提醒
   - 心理健康关注

4. **📊 市场分析** - 技术市场趋势洞察
   - 行业动态分析
   - 投资机会提醒
   - 技能市场需求
   - 竞品动态追踪

5. **💾 BWG 磁盘监控** - BandwagonHost VPS 磁盘使用监控
   - 每日自动检查磁盘使用情况
   - 磁盘空间警告和严重提醒
   - 详细的服务器状态报告
   - 流量使用情况统计

## 🚀 快速开始

### 1. 克隆项目

\`\`\`bash
git clone https://github.com/your-username/My-Jarvis.git
cd My-Jarvis
\`\`\`

### 2. 安装依赖

\`\`\`bash
npm install
\`\`\`

### 3. 配置环境变量

在 GitHub 仓库的 Settings > Secrets and variables > Actions 中添加以下环境变量：

#### 必需的环境变量

\`\`\`env
# Claude AI API 配置
ANTHROPIC_API_KEY=your-claude-api-key

# 邮件发送配置
EMAIL_SERVICE=gmail          # 邮件服务提供商
EMAIL_USER=your-email@gmail.com    # 发送邮箱
EMAIL_PASS=your-app-password       # 邮箱应用专用密码
NOTIFICATION_EMAIL=your-notification@gmail.com  # 接收邮箱

# GitHub API 配置 (可选，用于更好的工作提醒)
GITHUB_TOKEN=your-github-token

# BWG VPS 监控配置 (可选)
BWG_VEID=666722                    # BWG 服务器 VEID
BWG_API_KEY=your-bwg-api-key       # BWG API Key
BWG_DISK_WARNING_THRESHOLD=80      # 磁盘警告阈值 (%)，默认 80
BWG_DISK_CRITICAL_THRESHOLD=90     # 磁盘严重阈值 (%)，默认 90
BWG_SEND_DAILY_REPORT=false        # 是否发送每日正常报告，默认 false
\`\`\`

#### 获取 BWG API Key

1. 登录 [BandwagonHost 控制面板](https://bwh88.net/clientarea.php)
2. 进入 Services > My Services
3. 点击要监控的 VPS
4. 在 KiwiVM Control Panel 中找到 API Key
5. VEID 可以在 URL 或服务详情中找到

#### 获取 Claude API Key

1. 访问 [Anthropic Console](https://console.anthropic.com/)
2. 创建账户并获取 API Key
3. 将 API Key 添加到 GitHub Secrets

#### 配置邮件服务

**Gmail 配置示例：**
1. 开启两步验证
2. 生成应用专用密码
3. 使用应用专用密码作为 `EMAIL_PASS`

### 4. 启用 GitHub Actions

确保你的仓库启用了 GitHub Actions，工作流会在每天早上 8 点自动运行。

### 5. 手动测试运行

\`\`\`bash
# 测试邮件发送
npm run test:email

# 生成特定类型任务
npm run generate:work
npm run generate:learning
npm run generate:health
npm run generate:market

# 生成所有任务并发送汇总
npm run generate:all
npm run send:summary
\`\`\`

## 📁 项目结构

\`\`\`
My-Jarvis/
├── .github/
│   └── workflows/
│       └── daily-ai-tasks.yml      # GitHub Actions 工作流
├── scripts/
│   ├── ai-task-generator.js        # AI 任务生成核心
│   ├── email-sender.js             # 邮件发送模块
│   ├── generate-work-tasks.js      # 工作任务生成
│   ├── generate-learning-tasks.js  # 学习任务生成
│   ├── generate-health-tasks.js    # 健康任务生成
│   ├── generate-market-tasks.js    # 市场分析生成
│   ├── check-bwg-disk.js           # BWG VPS 磁盘监控
│   └── send-daily-summary.js       # 每日汇总发送
├── tasks/                          # 生成的任务文件存储
├── package.json
└── README.md
\`\`\`

## ⚙️ 自定义配置

### 修改执行时间

编辑 `.github/workflows/daily-ai-tasks.yml` 中的 cron 表达式：

\`\`\`yaml
schedule:
  # 每天早上 8:00 (UTC)，相当于北京时间 16:00
  - cron: '0 8 * * *'
\`\`\`

**时区转换参考：**
- 北京时间 08:00 = UTC 00:00 → \`'0 0 * * *'\`
- 北京时间 09:00 = UTC 01:00 → \`'0 1 * * *'\`

### 自定义提示词

你可以在各个生成脚本中修改 AI 提示词来调整生成的任务类型和风格：

- 工作提醒：\`scripts/generate-work-tasks.js\`
- 学习计划：\`scripts/generate-learning-tasks.js\`
- 健康建议：\`scripts/generate-health-tasks.js\`
- 市场分析：\`scripts/generate-market-tasks.js\`

### 邮件模板自定义

在 \`scripts/email-sender.js\` 中可以自定义：
- HTML 模板样式
- 邮件布局
- 品牌元素

## 🔧 高级功能

### 添加新的任务类型

1. 创建新的生成脚本 \`scripts/generate-custom-tasks.js\`
2. 在 \`daily-ai-tasks.yml\` 中添加新的执行步骤
3. 更新 \`send-daily-summary.js\` 以包含新类型

### 集成更多数据源

可以扩展系统以集成：
- 日历 API (Google Calendar)
- 健康数据 (Apple Health, Google Fit)
- 投资数据 (股票 API)
- 天气数据 (OpenWeatherMap)

### 智能通知策略

根据任务优先级和类型设置不同的通知策略：
- 高优先级任务立即通知
- 普通任务批量汇总
- 学习任务在空闲时间推送

## 🐛 故障排除

### 邮件发送失败

1. 检查邮箱配置和应用专用密码
2. 确认邮件服务商设置正确
3. 查看 GitHub Actions 运行日志

### AI 任务生成失败

1. 验证 Claude API Key 是否有效
2. 检查 API 配额是否充足
3. 查看错误日志确定具体问题

### GitHub Actions 不执行

1. 确保仓库启用了 Actions
2. 检查 workflow 文件语法
3. 确认分支设置和触发条件

## 📊 监控和分析

系统会自动记录：
- 任务生成成功率
- 邮件发送状态
- AI API 调用统计
- 用户参与度分析

查看生成的任务文件：\`/tasks/\` 目录下按日期保存。

## 🤝 贡献指南

欢迎提交 Issue 和 Pull Request！

1. Fork 项目
2. 创建功能分支
3. 提交更改
4. 推送到分支
5. 创建 Pull Request

## 📄 许可证

MIT License - 详见 [LICENSE](LICENSE) 文件

## 🎯 发展规划

- [ ] 增加 Web 界面控制台
- [ ] 支持多用户和团队协作
- [ ] 集成更多 AI 模型选择
- [ ] 添加任务完成反馈机制
- [ ] 支持多语言界面
- [ ] 移动端应用开发

---

**让 AI 成为你的贴身助手，提升每一天的效率！** 🚀

如有问题或建议，欢迎在 [Issues](https://github.com/your-username/My-Jarvis/issues) 中讨论。